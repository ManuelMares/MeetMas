...
let meet = new Meet("0", "0");

setTimeout(function() {
    meetId = window.location.href.substring(24);
    retrieveMeetFromStorage(meetId)
    .then(ans=>{
        meet.meetFromTableString(ans[meetId]);
        console.log(ans[meetId]);
        console.log(meet.toTableString());
...
async function startRegistering(){  
  //Check when the app loaded completely
...
async function addParticipant(participantContainer){
  ...
  if(buttons == null){
    ...
    meet.addParticipant(id, 0)  //function at main.js
  }
}
...
async function createNotification(type, id){
  ...
  addParticipation(id, value); //function at main.js
}
